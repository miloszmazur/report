...
\end{itemize}
Were tuned by using \textit{grid-search} algorithm with a number of possible values of $C_2$ and $C_3$. $C_1$, hovever, remained unchanged. The decision was made that, for the purposes of the competitive case, the number of resource markers should remain the deciding factor. % ask for clarification about that
The values of the parameters after grid-search tuning are presented in table \ref{table:x tunedfitnessfunctionparameters}.
\begin{table} [h]
    \centering
    \begin{tabular} {c c}
        \hline \hline
    ...
    \end{tabular}
    \caption{Selected Fitness Function Factor Weights}
    \label{table:x tunedfitnessfunctionparameters}
\end{table}
